    ...
    if (results.length === 0) {
        ...
        results.forEach(result => {
            ...
            if (result.statusId == 139) {
                console.log("fix");
                resultTr.innerHTML = `
                    <td style="font-weight:600;">${result.order.orderId} </td>
                    <td style="color:green;"><a>${result.order.orderName} </a></td>
                ...
                resultTr.innerHTML = `
                    <td style="font-weight:600;">${result.order.orderId} </td>
                    <td><a style="color:green;" href="detail_order.html?orderId=${result.order.orderId}" target="blank">${result.order.orderName} </a></td>
                    <td><a style="color:blue;" href="../public/view_driver_infor.html?driverId=${result.order.ownedVehicleInfor.driverId}" target="blank">${result.order.ownedVehicleInfor.driver.fullName} - ${result.order.ownedVehicleInfor.vehicle.vehicleName} - ${result.order.ownedVehicleInfor.description}</a></td>
                `;

                var td = document.createElement("td");
                if (result.statusId == 12)
                    td.innerHTML = `<button style="background-color:#5ad7ff;border:none;padding:7px 10px;color:#fff;font-weight:600;cursor:pointer;font-weight:600;border-radius:7px;margin-bottom:6px" onclick="createRate(${result.order.orderId},${result.order.ownedVehicleInfor.driverId})">Đánh giá</button>
                        <div style="text-align:center;">
                            <div style="font-weight:bolder;">1   2   3   4   5</div>
                            <div>
                        ...
                        <textarea style="border:#ccc;background-color: #eee;box-shadow: 0 0 0 3px hsla(var(--input-focus-h), var(--input-focus-s), calc(var(--input-focus-l) +
                        40%), 0.8);" type="text" id="c${result.order.orderId}" />`;

                else
                    td.innerHTML = `<p style="color:#000; font-weight:600;"><font>Đã đánh giá</font></p>`;
...
function displayOnWorkedOrder(results) {
    ...
    `;
        ...
        results.forEach(result => {
            ...
            var resultTr = document.createElement("tr");
            resultTr.innerHTML = `
                <td style="font-weight:bolder;">${result.order.orderId} </td>
                <td><a style="color:green;" href="detail_order.html?orderId=${result.order.orderId}" target="blank">${result.order.orderName} </a></td>
                <td><a style="color:#29caff;" href="../public/view_driver_infor.html?driverId=${result.order.ownedVehicleInfor.driverId}" target="blank">${result.order.ownedVehicleInfor.driver.fullName} - ${result.order.ownedVehicleInfor.vehicle.vehicleName} - ${result.order.ownedVehicleInfor.description}</a></td>
                <td style="font-weight:600">${result.status} </td>
            `;
            var td = document.createElement(td);
            if (new Date(result.order.arrivedDate) <= new Date()) {
                td.innerHTML = `<p style="text-align:center;margin-top:40px;"><font color="red">${result.order.arrivedDate}</font></p>`;
            }
            else {
                td.innerHTML = `<p style="text-align:center;margin-top:40px;"><font color="green">${result.order.arrivedDate}</font></p>`;
            }
            resultTr.appendChild(td);
            var button = document.createElement("td");
            if (result.statusId == 6) {
                button.innerHTML = `<td><p>Hãy thanh toán để xác nhận lấy hàng</p>
            <button style="background-color:#f44336;border:none;padding:10px 5px;border-radius:7px;color:#fff;font-weight:bolder;cursor:pointer;" onclick="updateStatus(8,${orderId})">Chưa thể nhận</button></td>
            `;
            }
            ...
            `;
                ...
                button.innerHTML = `<td><button style= "background-color:#f44336;border:none;padding:7px 7px;color:white;font-weight:600;border-radius:5px;cursor:pointer;" onclick="updateStatus(12,${orderId})">Xác nhận kết thúc đơn hàng</button><td>
            `;
            }
            else {
...
function displayWaitedDeliveredOrder(results) {
    ...
    if (results.length === 0) {
        waitedOrder.innerHTML = `
            ...
            <tr>
                <td colspan="3">Bạn chưa có đơn hàng nào xác nhận</td>
            </tr>
        `;
    }
    else {
        ...
        results.forEach(result => {
            var resultTr = document.createElement("tr");
            resultTr.innerHTML = `
                <td style="font-weight:bolder;">${result.orderId} </td>
                <td><a style="color:green;" href="detail_order.html?orderId=${result.orderId}" target="blank">${result.orderName} </a></td>
                <td><a style="color:#27caff;" href="../public/view_driver_infor.html?driverId=${result.ownedVehicleInfor.driverId}" target="blank">${result.ownedVehicleInfor.driver.fullName} - ${result.ownedVehicleInfor.vehicle.vehicleName} - ${result.ownedVehicleInfor.description}</a></td>
                <input type="hidden" id="${result.orderId}" value= "${result.ownedVehicleInfor.oVIId}"/>
                <td><button style="background-color: #f14141; color:white; border:none;padding:10px 30px;border-radius:7px;font-weight:600;cursor:pointer;" onclick="xoaTaiXe(${result.orderId})">Xóa tài xế</button></td>
            `;
...
function displayInitOrder(results) {
    ...
    if (results.length === 0) {
        ...
        results.forEach(result => {
            ...
            resultTr.innerHTML = `
                <td style="font-weight:700;">${result.orderId} </td>
                <td><a style="color:green;font-size:20px;" href="detail_order.html?orderId=${result.orderId}" target="blank">${result.orderName} </a></td>
            `;
            ...
            fetch("https://localhost:7156/api/Customer/WishAcceptDriverList/" + result.orderId, {
                ...
                .then(data => {
                    ...
                    if (dataLength >= 1) {
                        ...
                        deleteButton.innerHTML = `<button style="background-color: #f14141; color:white; border:none;padding:10px 30px;border-radius:7px;font-weight:600;cursor:pointer;" onclick="xoaDon(${orderId})">Xóa</button>`;
                        td2.innerHTML = `<button style="background-color: #2ed6c9; color:white; border:none;padding:10px 30px;border-radius:7px;font-weight:700;cursor:pointer;" onclick="chonTaiXe(${orderId})">Chọn tài xế</button> `;
                        ...
                        var deleteButton = document.createElement("td")
                        deleteButton.innerHTML = `<button style="background-color: #f14141; color:white; border:none;padding:10px 30px;border-radius:7px;font-weight:600;cursor:pointer;" onclick="xoaDon(${orderId})">Xóa</button>`;
                        chuaCoTX.innerHTML = `<p style="color:black;font-weight:bolder;">Chưa có tài xế</p>`;
